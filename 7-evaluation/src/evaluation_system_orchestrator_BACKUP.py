...
class EvaluationSystemOrchestrator:
    ...
    def create_tables(self):
        ...
        """
        # MODIFIED: Added classifier_id to the table
        query_classifier = """
    ...
    def handle_message(self, label_dict):
        ...
        if self.waiting_for_human:
            print(f"[WARNING] Buffer locked. Waiting for decision on {self.current_pending_batch}. Ignoring new payload.")
            return False
        ...
        if source == "expert":
            ...
            cursor = conn.cursor()
            cursor.execute("SELECT COUNT(*) FROM expertLabelTable")
            expert_count = cursor.fetchone()[0]
            conn.close()
            
            BATCH_SIZE = self.config.get("evaluation_batch_size", 5) 
            
            if expert_count >= BATCH_SIZE:
                batch_name = f"Batch_{player_id}"
                print(f"\nSufficient labels reached (Batch of {BATCH_SIZE}). Triggering report...")
                ...
                if self.service_mode:
                    ...
                    # Temporarily unlock to let the auto-decision through
                    ...
                    # LOCK THE STATE FOR HUMAN INPUT
                    self.waiting_for_human = True
                    self.current_pending_batch = batch_name
    ...
    def submit_human_decision(self, batch_id, classifier_is_good):
        ...
        if classifier_is_good:
            ...
            # Extract JSON data for the messaging payload
            ...
            # --- MODIFIED: Print clean console message and save output as JSON file ---
    ...
    def start_server(self):
        ...
        self.server.api.add_resource(
            ReceiveJsonApi, "/evaluation",
            resource_class_kwargs={'json_schema_path': LABEL_PATH_SCHEMA_REL, 'handler': self.handle_message}
        )
        self.server.api.add_resource(
...
#             print("\n[SIMULATED OUTBOUND TO MESSAGING SYSTEM]")
#             print(json.dumps(recalibration_config, indent=4))
#             print("----------------------------------------")
