...
def _to_numpy(X):
    """Always return a plain ndarray — prevents sklearn feature-name warning."""
...
class ClassifierController:
    ...
    def _load_latest_classifier(self):
        ...
        try:
            with open(LATEST_CLASSIFIER_PATH, "r", encoding="utf-8") as f:
    ...
    def save_uploaded_classifier(self, uploaded_file):
        ...
        with open(LATEST_CLASSIFIER_PATH, "w", encoding="utf-8") as f:
    ...
    def classify(self, prepared_session: dict):
        ...
        # ← convert to numpy so the model never sees feature names
        predicted_rating = int(self.model.predict(_to_numpy(input_frame))[0])
