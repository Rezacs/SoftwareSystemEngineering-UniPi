...
class SegregationSystemOrchestrator:
    ...
    def _with_legacy_payload(self, payload: dict) -> dict:
        normalized_payload = dict(payload)
        normalized_payload.setdefault("balancing_report", {})
        normalized_payload.setdefault("coverage_report", {})
        normalized_payload.setdefault(
            "learning_sets",
            normalized_payload.get("calibration_set", [])
        )
    ...
    def run(
        ...
        if state["phase"] == "waiting_balancing_decision":
            ...
            if balancing_decision is None:
                return self._with_legacy_payload({
